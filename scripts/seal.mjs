...
const audio = await readFile(new URL('../private/joke.mp3',import.meta.url));
const audioIv = webcrypto.getRandomValues(new Uint8Array(12));
const sealedAudio = await webcrypto.subtle.encrypt({name:'AES-GCM',iv:audioIv},key,audio);
await writeFile(new URL('../dist/voice.bin',import.meta.url),Buffer.concat([Buffer.from(audioIv),Buffer.from(sealedAudio)]));
...
console.log('История и запись зашифрованы. Код не сохранён.');
